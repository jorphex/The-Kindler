...
async def get_next_user_number_async():
    """Async wrapper to increment the counter exactly once per call."""
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(None, _increment_and_get_user_count_from_file)
...
def create_cover_sync(title: str, domain: str, top_image_url: str = None):
    ...
    try:
        ...
        if os.path.exists(cover_filename):
            os.remove(cover_filename) # Cleanup partial
        if temp_dl_image_path and os.path.exists(temp_dl_image_path):
            os.remove(temp_dl_image_path)
        return None
...
def create_epub_sync(title: str, content_html: str, cover_path: str, domain: str, source_url: str, top_image_url: str = None):
    ...
    try:
        ...
        chapter_html_content = f"<h1>{escaped_title}</h1>"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    ebooks_created_count = 0
    kindle_sent_count = 0
    telegram_uploaded_count = 0
    loop = asyncio.get_running_loop()
    ...
    for i, url_to_process in enumerate(valid_urls):
        ...
        try:
            ...
            if result["status"] == "error":
                ...
                try:
                    with open(epub_path_res, 'rb') as epub_file:
                        await update.message.reply_document(document=epub_file)
                    telegram_uploaded_count += 1
    ...
    if ebooks_created_count == 0 and len(valid_urls) > 0:
         await update.message.reply_text("Finished processing. No ebooks were successfully created.")
    elif ebooks_created_count > 0:
         await update.message.reply_text(
             "Finished processing "
             f"{ebooks_created_count}/{len(valid_urls)} URL(s). "
             f"Kindle sends: {kindle_sent_count}. Chat uploads: {telegram_uploaded_count}."
         )


    # --- Post-Processing (Milestones, Donations) ---
    if kindle_sent_count > 0:
        context.user_data.setdefault('ebook_count', 0)
        context.user_data['ebook_count'] += kindle_sent_count
        ...
        for milestone_val in sorted(MILESTONES):
            if user_total_ebook_count >= milestone_val and (user_total_ebook_count - kindle_sent_count) < milestone_val:
        ...
        if user_total_ebook_count >= next_donation_trigger:
            ...
            await update.message.reply_html(
                "Love turning articles into Kindle ebooks?<br/>"
                "Help keep this bot running and improving.<br/>"
                "Send a tip (ETH/Base/Polygon/ARB):<br/>"
                "<code>0xaeAe4F2b0a2958e2dCC58b7F5494984B8e375369</code><br/>"
